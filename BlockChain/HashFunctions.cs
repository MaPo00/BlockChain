...
using Force.Crc32;
    ...
    public class SHA256Hash : IHashFunction
    {
        public string GetHash(string data)
        {
            var sha = SHA256.HashData(Encoding.ASCII.GetBytes(data));
    ...
    public class CRC32Hash : IHashFunction
    {
        public string GetHash(string data)
        {
            ...
            return DecimalToArbitrartSystem(hash, 64);

        }

        private static string DecimalToArbitrartSystem(long decimalNumber, int radix)
        {
            ...
            {
                throw new ArgumentException("radix must be >=2 and <=" + Digits.Length.ToString());
            }
